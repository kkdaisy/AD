...
# 이벤트 로그 이름과 경로 설정
$eventLogName = "Application" # 이벤트 로그 이름 (Application, Security, System)
...
foreach ($serverName in $servers) {
    ...
    $warningErrorEvents = Invoke-Command -ComputerName $serverName -Credential $credential -ScriptBlock {
        ...
        Get-WinEvent -LogName $logName | Where-Object { ($_.TimeCreated -ge $start) -and ($_.TimeCreated -le $end) -and ($_.Level -ge 3) }
...
# 결과 배열을 CSV로 내보내기
$allWarningErrorEvents | Export-Csv -Path $exportPath -NoTypeInformation 
