...
$exportPath = "C:\Mylog\ExportedLog_server_errors_warnings.csv" # 내보낼 CSV 파일 경로 및 이름
...
foreach ($serverName in $servers) {
    ...
    $events = Invoke-Command -ComputerName $serverName -Credential $credential -ScriptBlock {
        ...
        Get-WinEvent -LogName $logName | Where-Object { ($_.TimeCreated -ge $start) -and ($_.TimeCreated -le $end) -and (($_.LevelDisplayName -eq "Error") -or ($_.LevelDisplayName -eq "Warning")) }
...
Write-Host "Error 및 Warning 레벨의 이벤트 로그를 CSV 파일로 내보냈습니다: $exportPath"
